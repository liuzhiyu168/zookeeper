...
public class ZooKeeperQueue {
    ...
    public ZooKeeperQueue(String queueName) throws IOException, KeeperException, Exception {
        ...
        do {
            synchronized (mutex) {
                ...
                if (nodes == null || nodes.size() == 0) {
                    ...
                    System.out.print("=============消费 consume a message from queue：" + first);
    ...
    public void produce(byte[] data) throws KeeperException, InterruptedException, Exception {
        ...
        System.out.print(Thread.currentThread().getName() + " ");
        System.out.print("===========生产 produce a message to queue：" + node);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < 10; i++) {
                    try {
                        Thread.sleep(1000);
                        queue.produce(("我是刘大大" + i).getBytes());
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    } catch (KeeperException e) {
                        e.printStackTrace();
